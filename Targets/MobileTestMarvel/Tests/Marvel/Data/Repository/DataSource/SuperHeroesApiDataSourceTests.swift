...
class SuperHeroesApiDataSourceTests: TestCase {
    var sut: SuperHeroesApiDataSource!
    var restApiMock: SuperHeroesRestApiMock!

    override func setUp() {
        super.setUp()
        restApiMock = SuperHeroesRestApiMock()
        sut = SuperHeroesApiDataSource(restApi: restApiMock)
    }
    ...
    func testDetailSuccess() {
        restApiMock.detailSuccess = true
    ...
    func testDetailError() {
        restApiMock.detailSuccess = false
        sut.detail(id: "1") { result in
            switch result {
            case .success:
                XCTAssertThrowsError("should never been called")
